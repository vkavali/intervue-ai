...
import { generateMockPack } from "@/lib/ai-mock-pack";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pack = await generateMockPack({
      targetRole,
      interviewType,
      seniority,
      industry,
    ...
    console.error("POST /api/candidate/mock-pack error:", error);
    const message =
      error instanceof Error ? error.message : "Failed to generate mock pack";
    const status =
